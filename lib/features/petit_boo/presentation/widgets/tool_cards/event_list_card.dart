...
class _EventItemCard extends StatelessWidget {
  ...
  Activity _toActivity(BuildContext context) {
    ...
    final pricingTypeValue = _getStringValue([
      'discovery_pricing_type',
      'discoveryPricingType',
    ]);
    final isDiscovery = bookingMode == 'discovery' || pricingTypeValue != null;
    final isFree = isDiscovery
        ? pricingTypeValue == 'free'
        : _getIsFree(priceMin, priceMax);
    final discoveryPricingType = switch (pricingTypeValue) {
      'free' => DiscoveryPricingType.free,
      'paid' => DiscoveryPricingType.paid,
      _ => null,
    };
