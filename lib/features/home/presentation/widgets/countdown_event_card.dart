...
Widget _buildActivityPrice(
  BuildContext context,
  Activity activity, {
  required TextStyle freeStyle,
  required TextStyle paidStyle,
  bool shortLabel = false,
}) {
  // Discovery pricing is classified by the API. Numeric values are only
  // indicative and must never turn an unknown/paid discovery event into free.
  if (!activity.isBookingActivity) {
    if (!activity.isFreeDiscovery) return const SizedBox.shrink();
    return Text(context.l10n.commonFree, style: freeStyle);
  }

  final minimum = activity.priceMin;
  if (minimum == null || minimum == -1) return const SizedBox.shrink();

  final price = minimum > 0 ? minimum : (activity.priceMax ?? 0);
  if (price <= 0) {
    // Booking events still use the API's `is_free` signal because some paid
    // personalized events arrive with zero numeric prices.
