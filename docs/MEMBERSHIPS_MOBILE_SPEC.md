...
| Topic | Note |
|---|---|
| **Members-only badge on event cards** | Mobile event cards display a "Privé 🔒" badge when `event.is_members_only == true`. **The field is computed from `visibility`** — any visibility outside `Public` / `PublicProtected` (i.e. `Unlisted`, `Private`, `PrivateProtected`) makes the event members-only. The raw `events.is_members_only` column is legacy and not load-bearing; trust the resource field. |
| **Per-event access gates and bypass** | `GET /api/v1/events/{uuid}` runs two gates after fetching the event: (1) **password gate** for `*_protected` visibilities, (2) **members-only gate** when `isMembersOnly()` returns true. Both gates are bypassed for the org's owner and active members — they don't need the password and they don't need to be in `organization_members` (owner is exempt by virtue of `Organization.user_id`). Non-members can pass the password gate but still hit the members-only gate when visibility is non-public. |
